...
export function renderLanguagesSvg(data, theme, options = {}) {
  ...
  // Calculate segmented progress bar widths (total available width = 455px)
  const totalBarWidth = 455;
  let currentX = 0;
  const barSegments = languages.map((lang) => {
    const width = Math.max(2, (lang.percent / 100) * totalBarWidth);
    const segment = {
      x: currentX,
      width,
      color: lang.color,
  ...
  // Calculate card height based on language count and progress bar
...
<svg width="495" height="${cardHeight}" viewBox="0 0 495 ${cardHeight}" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Top Languages for ${escapeXml(username)}">
  <defs>
    ...
    <clipPath id="progressBarClip">
      <rect width="455" height="9" rx="4.5" />
    </clipPath>
    ...
    <style><![CDATA[
      ...
      .lang-name {
        font: 600 12.5px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;
        fill: ${theme.text};
      }
      .lang-pct {
  ...
  <rect x="0.5" y="0.5" rx="14" width="494" height="${cardHeight - 1}" fill="url(#bgGrad)" stroke="${borderStroke}" stroke-width="1.2" />
  ...
  <g transform="translate(20, 46)">
    <line x1="0" y1="0" x2="455" y2="0" stroke="url(#dividerGrad)" stroke-width="1.2" stroke-opacity="0.8" />
  </g>
  ...
  <g transform="translate(20, 56)" class="stagger-bar">
    <g clip-path="url(#progressBarClip)">
      <rect width="455" height="9" fill="${theme.badgeBg}" />
      ${barSegments
        .map(
          (seg) =>
            `<rect x="${seg.x}" y="0" width="${seg.width}" height="9" fill="${seg.color}" />`
        )
        .join('\n      ')}
    </g>
  </g>
  ...
  <!-- Language Grid List (2 Columns of up to 3 Rows) -->
  <g transform="translate(20, ${hideProgress ? 56 : 74})">
    ${languages
      .map((lang, index) => {
    ...
    <g transform="translate(${x}, ${y})" class="${staggerClass}">
      <rect width="220" height="36" rx="8" fill="url(#tileGrad)" stroke="${borderStroke}" stroke-opacity="0.5" stroke-width="1"/>
      <!-- Language Color Bullet -->
      <circle cx="16" cy="18" r="5" fill="${lang.color}" />
      <text x="28" y="22" class="lang-name">${escapeXml(lang.name)}</text>
      <text x="206" y="22" text-anchor="end" class="lang-pct">${lang.formattedPercent}</text>
    </g>`;
      })
      .join('\n')}
  </g>
...
export function renderLanguagesErrorSvg(message, theme) {
  ...
  return `
<svg width="495" height="225" viewBox="0 0 495 225" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Error: ${safeMessage}">
  <defs>
  ...
  <!-- Background Box -->
  <rect x="0.5" y="0.5" rx="14" width="494" height="224" fill="url(#errBgGrad)" stroke="${borderStroke}" stroke-width="1.2" />
  ...
  <g transform="translate(24, 70)">
    <rect width="447" height="80" rx="8" fill="${theme.badgeBg}" stroke="${borderStroke}" stroke-opacity="0.7" stroke-width="1" />
    <text x="16" y="32" class="error-msg">${safeMessage}</text>
    <text x="16" y="56" class="error-sub">Please verify your username or check your GH_TOKEN on Vercel.</text>
  </g>

  <text x="24" y="195" class="error-sub">Tip: Pass ?username=your_github_handle</text>
</svg>
`.trim();
}
