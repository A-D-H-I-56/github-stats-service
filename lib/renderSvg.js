...
export function renderStatsSvg(stats, theme) {
  ...
  return `
<svg width="495" height="265" viewBox="0 0 495 265" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="GitHub Stats for ${escapeXml(username)}">
  <defs>
    ...
    <style><![CDATA[
      ...
      .stagger-3 {
        animation: fadeIn 0.4s ease-out 0.19s both;
      }
      .stagger-4 {
        animation: fadeIn 0.4s ease-out 0.26s both;
      }
      .stagger-5 {
        animation: fadeIn 0.4s ease-out 0.33s both;
      }
  ...
  <rect x="0.5" y="0.5" rx="14" width="494" height="264" fill="url(#bgGrad)" stroke="${borderStroke}" stroke-width="1.2" />
  ...
  <g transform="translate(20, 126)" class="stagger-3">
    ...
    <text x="100" y="45" class="stat-sub">(${formatNumber(publicRepos)} Public / ${formatNumber(privateRepos)} Private)</text>
  </g>
  ...
  <g transform="translate(20, 194)" class="stagger-5">
    <rect width="455" height="58" rx="9" fill="url(#tileGrad)" stroke="${borderStroke}" stroke-opacity="0.6" stroke-width="1"/>
    ...
    <text x="52" y="23" class="stat-label">PRS &amp; ISSUES CONTRIBUTED</text>
    <text x="52" y="45" class="stat-val">${formatNumber(prsAndIssues)}</text>
    <text x="125" y="45" class="stat-sub">(${formatNumber(totalPullRequests)} Pull Requests · ${formatNumber(totalIssues)} Issues · ${formatNumber(totalReviews)} Reviews)</text>
  </g>
</svg>
`.trim();
}
...
export function renderErrorSvg(message, theme) {
  ...
  return `
<svg width="495" height="265" viewBox="0 0 495 265" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Error: ${safeMessage}">
  <defs>
  ...
  <!-- Background Box -->
  <rect x="0.5" y="0.5" rx="14" width="494" height="264" fill="url(#errBgGrad)" stroke="${borderStroke}" stroke-width="1.2" />
  ...
  <g transform="translate(24, 70)">
    <rect width="447" height="90" rx="8" fill="${theme.badgeBg}" stroke="${borderStroke}" stroke-opacity="0.7" stroke-width="1" />
    <text x="16" y="32" class="error-msg">${safeMessage}</text>
    <text x="16" y="58" class="error-sub">Please verify your username or check your GH_TOKEN on Vercel.</text>
  </g>

  <text x="24" y="235" class="error-sub">Tip: Pass ?username=your_github_handle</text>
</svg>
`.trim();
}
